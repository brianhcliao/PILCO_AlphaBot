...
if exist('plotting', 'var') && isfield(plotting, 'verbosity') && plotting.verbosity > 0
  if ~ishandle(3);
      figure(3); 
  else
  ...
  drawnow;
  xlabel('time step'); 
  ylabel('immediate cost');
end

% 1. Update the policy
opt.fh = 1;
[policy.p fX3] = minimize(policy.p, 'value', opt, mu0Sim, S0Sim, dynmodel, policy, plant, cost, H);

% (optional) Plot overall optimization progress
if exist('plotting', 'var') && isfield(plotting, 'verbosity') && plotting.verbosity > 1
  if ~ishandle(2); 
      figure(2); 
  else set(0,'CurrentFigure',2); 
  end
  hold on; 
  plot(fX3); 
  drawnow; 
  xlabel('line search iteration'); 
