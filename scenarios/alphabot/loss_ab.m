%% loss_ab.m
% *Summary:* AlphaBot loss function; the loss is 
% $1-\exp(-0.5*d^2*a)$,  where $a>0$ and  $d^2$ is the squared difference
% between the actual and desired position of AlphaBot. 
...
%
%
%   function [L, dLdm, dLds, S2] = loss_ab(cost, m, s)
%
%
% *Input arguments:*
%
...
for i = 1:length(cw)                    % scale mixture of immediate costs
    cost.z = target; 
    cost.W = Q/cw(i)^2;
  [r rdM rdS s2 s2dM s2dS] = lossSat_ab(cost, M, S);
