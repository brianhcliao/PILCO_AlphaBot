...
% 
% For AlphaBot scenario,
% the cost is added by 1 when the velocity state constraint is violated.
%
...
function [L, dLdm, dLds, S, dSdm, dSds, C, dCdm, dCds] = lossSat_ab(cost, m, s)
%% Code
...
if isfield(cost,'W')        % cost width
    W = cost.W; 
else
    W = eye(D);
end

if isfield(cost,'z')        % cost target
    z = cost.z; 
else
    z = zeros(D,1); 
end
...
% 1a. Velocity constraints
% exd = abs(m(2)) - 3;
% if exd > 0
%   L = L + exd^2;
% elseif abs(L) < 0.000001
%   L = L + 0.01;
% end

% 1b. derivatives of expected cost
if nargout > 1
...
if nargout > 6
    ...
    C = L*t;
    dCdm = t*dLdm - L*iSpW;
    dCds = -L*(bsxfun(@times,iSpW,permute(t,[3,2,1])) + bsxfun(@times,permute(iSpW,[1,3,2]),t'))/2;
