...
if nargin < 3
    iter = [-500 -1000]; 
end           % default training iterations

D = size(gpmodel.inputs,2); 
...
for i = 1:E                                          % train each GP separately
  fprintf('GP %i/%i\n', i, E);
  try   % BFGS training
    [gpmodel.hyp(:,i) v] = minimize(lh(:,i), @hypCurb, iter(1), covfunc, gpmodel.inputs, gpmodel.targets(:,i), curb);
  catch % conjugate gradients (BFGS can be quite aggressive)
    [gpmodel.hyp(:,i) v] = minimize(lh(:,i), @hypCurb, struct('length', iter(1), 'method', 'CG', 'verbosity', 1), covfunc, gpmodel.inputs, gpmodel.targets(:,i), curb);
  end
  nlml(i) = v(end);
end
